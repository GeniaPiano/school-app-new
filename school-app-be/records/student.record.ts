...
export class StudentRecord implements StudentEntity {
    ...
    constructor(obj: StudentRecord) {
        ...
        if (!obj.password || obj.password.length <= 7 || obj.password.length > 40) {
            throw new ValidationError('Student should contain from 8 to 40 characters');
        }
    ...
    async insert():Promise<string>  {
        ...
        await pool.execute("INSERT INTO `students` VALUES(:id, :name, :last_name, :email, :password)", {
            id: this.id,
            name: this.name,
